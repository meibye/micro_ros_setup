  ...
  if [ -f build/micro-ROS.bin ]; then
    ...
    if [$USE_STFLASH = true]; then 
      ...
      # TODO: add stlink v3, should it be stlink.cfg ?
        echo "Error. Unsupported OpenOCD USB programmer"
        exit 1
      fi

      openocd -f $ST_INTERFACE -f target/stm32l4x.cfg -c init -c "reset halt" -c "flash write_image erase build/micro-ROS.bin 0x08000000" -c "reset" -c "exit"
    fi
  else
